...
The primary purpose of the script is to identify clusters of singularity dots and apply various filtering algorithms to simplify their spatial structure. These singularities often appear in complex optical fields such as those generated by Laguerre-Gaussian beams. The filtering techniques implemented here allow for the reduction of noise and isolation of relevant clusters, aiding in the analysis and visualization of these singularities.

## Main Features

1. **Cluster Identification and Filtering**: The script provides several functions to identify and filter clusters of singularity dots based on their neighborhood configuration. Filters include identifying dots with zero, one, two, three, or specific planar neighbors.

2. **Customizable Filtering Process**: The script includes a global filtering function that applies all the filters sequentially to simplify the dot dictionary.

3. **Interactive Visualization**: The script allows for saving and visualizing the singularity dots, which is helpful for understanding the spatial distribution of singularities.

## Key Functions

### filter_PlaneTwoClusters
Identifies and filters out two-dot horizontal clusters in three planes (x, y, z) from a given dictionary of dots. This helps to simplify the field by averaging the positions of clustered singularities.

### filter_PlaneTwoClusters_Vertical
Placeholder function for future development to handle vertical clusters specifically.

### globalFilterDots
Applies all the available filtering functions to the input dictionary of dots. It helps in reducing the complexity of the dot field by filtering out unwanted clusters and isolated dots.

### filtered_dots
Wrapper function for `globalFilterDots` that combines all filtered dots into a single array, making it easier to analyze the final, filtered set of singularities.

## Usage Example
The script is intended to be used as a part of a larger analysis pipeline for optical field singularities. Users can call `globalFilterDots` to apply all filters and then use the `filtered_dots` function to obtain the processed output. The functions are particularly useful in the context of complex optical systems, such as those involving knot-like singularities or structured beams.
