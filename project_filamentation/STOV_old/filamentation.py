...
# %% Resolutions
xResolution = 141 # N_per
yResolution = 141
tResolution = 141
loopInnerResolution, loopOuterResolution = 521, 1  # M, Kmax
zResolution = loopInnerResolution * loopOuterResolution
xStart, xFinish = 0, 4500 * 1e-6  # 10
yStart, yFinish = 0, 4500 * 1e-6  # 10
zStart, zFinish = 0, 1.0
tStart, tFinish = 0, 400 * 1e-12

# temp
sigma_K8 = 3.700000000000001e-128
sigma = 5.132934333616703e-24
rho_at = 5e+24
a = 5e-13
tau_c = 3.5e-13
###
...
# %% temporal
C = 0  # chirp # TEMPORAL ################################################################################
n2 = 5.57e-19 * 1e-2 ** 2  # 3 * chi3 / (4 * eps0 * c * n0 ** 2) # TEMPORAL #######################################################
...
def Betta_func(K):
    Betta = [0, 0 * 2e-0, 2, 3, 4, 5, 6, 7, 3.79347046850176e-121]
    return Betta[K]
...
def ADI_2D1_nonlinear(E0, loopInnerM, loopOuterKmax):
    ...
    plasmaDensity = plasma_density(E) * 0
...
if __name__ == '__main__':
    if module_2D:
        ...
        if module_ADI:
            ...
            plt.plot(tArray, np.abs(fieldAdi)[0, :] ** Int, ls='-', label='ADI', color='red', lw=10)
        if module_Paraxial:
