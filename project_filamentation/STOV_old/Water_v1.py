...
# %% Resolutions
xResolution = 181 # N_per
yResolution = 181
tResolution = 191
loopInnerResolution, loopOuterResolution = 501, 1  # M, Kmax
zResolution = loopInnerResolution * loopOuterResolution
xStart, xFinish = 0, 600 * 1e-6  # 10
yStart, yFinish = 0, 600 * 1e-6  # 10
zStart, zFinish = 0, 0.10
tStart, tFinish = 0, 1200 * 1e-15
...
def Field_STOV_simple(x, y, t):
    ...
    return Imax * (((t - t0)/tSTOVRadius + 1j * np.sign(lOAM) * (x - x0) / xSTOVRadius) ** (np.abs(lOAM)) * y_dependence(y - y0) *
...
def Field_STOV_simple0(x, y, t):
    ...
    return Imax * (((t - t0)/tSTOVRadius + 1j * np.sign(lOAM) * radius(x - x0, y - x0) / xSTOVRadius) ** (np.abs(lOAM)) * y_dependence(y - y0) *
            x_dependence(x - x0) * t_dependence(t - t0) *np.exp( -1j * k0 * radius(x - x0, y - x0) ** 2 / (2 * f))
            )
...
def split_step_old_time(shape, loopInnerM=1, loopOuterKmax=1):
    ...
    def plasma_density0(E):
        ...
        # print((abs(E)** (2 * K)).max())
        for i in range(tResolution):
            plasmaDensity[:, :, i] = 2 * (sigma_K8 * abs(E[:, :, int(tResolution/2)]) ** (2 * K) * (np.sqrt(np.pi / (8 * K)))
                                      * rho_at * tFinish * 0.1)
...
if __name__ == '__main__':
    if module_1D:
        ...
        if module_ADI:
            fieldAdi = ADI_2D1_nonlinear(Field_1_2D(xtMesh[0], xtMesh[1]),
                                         loopInnerResolution, loopOuterResolution)
            plt.plot(tArray, np.abs(fieldAdi)[0, :] ** Int, ls='-', label='ADI', color='red', lw=4)
        if module_Paraxial:
            ...
            plot_1D(tArray, np.abs(fieldOLD[int(xResolution / 2), int(yResolution / 2), :] ** Int)
                    , label='Paraxial', xname=r'$t$', yname='', color='lime', ls='-', lw=3)
            plt.show()
        fig, ax = plt.subplots(figsize=(8, 7))
        if module_ADI:
            plt.plot(xArray, np.abs(fieldAdi[:, int(tResolution / 2)]) ** Int, color='red', ls='-', lw=4)
        if module_Paraxial:
            plot_1D(xArray - x0, np.abs(fieldOLD[:, int(yResolution / 2), int(tResolution / 2)]) ** Int,
                     label='Paraxial', color='lime', ls='--', lw=3)
        plt.show()
    if module_2D:
        E = Field_1(xytMesh[0], xytMesh[1], xytMesh[2])[:, :, int(tResolution / 2)]
        Sq = np.sum(E ** 2) * (xArray[1] - xArray[0]) * (yArray[1] - yArray[0])
        ...
        """fieldAdiZ = ADI_2D1_nonlinear_Z(Field_1_2D(xtMesh[0], xtMesh[1]),
                                     loopInnerResolution, loopOuterResolution)"""
        fieldOLD = split_step_old_time_Z(Field_1, loopInnerResolution, loopOuterResolution)
        plot_2D(np.abs(fieldOLD[:, int(yResolution/2), :]) ** Int, zArray * 1e3, xArray * 1e3, 'z (mm)', 'x (mm)',
                map='viridis')
        #plt.xlim(2.25 - 1, 2.25 + 1)
        #plt.ylim(2.25 - 1, 2.25 + 1)
        #plt.title(f'z=1m, t={round((tArray[TIME] + (tArray[0] - tArray[-1]) / 2) * 1e12, 1)}ps', fontweight="bold",
        #          fontsize=26)
        plt.show()
        plt.close()
        #np.save('filam4_t_v2.npy', fieldOLD)
        plt.show()
        exit()
