"""
Same framework as HOBBIT_underwater_filamentation_along_CW.py, extended to support pulsed beams
(with finite duration) and time‐axis diagnostics.

Key differences from the previous version:
  • Beam generators now accept time (t) explicitly, enabling spatio‑temporal pulses
      – Hobbit, asymmetric LG, and STOV variants include temporal envelopes (tau_0, t0)
  • split_step_time solver records and plots evolution along the time axis
  • Example “__main__” workflow demonstrates:
      1. Generating a time‑dependent field on an (x, y, t) mesh
      2. Normalizing to target power Pmax
      3. Propagating with split_step_time over z steps
      4. Visualizing x–t slices via plot_2D (x vs. t) at fixed y
  • module_checking_spectrum now offers temporal profile plots (E vs. t) and ω‑spectrum

Usage:
  • Adjust top‑of‑file flags (module_checking_spectrum, module_intensity), grid
    & pulse parameters (t_start, t_finish, tau_0), and dz as needed.
  • Define or import a pulsed beam function (hobbit, asymmetric_lg, etc.).
  • Call split_step_time(field_fn, M, K) to simulate.
  • Use plot_1D / plot_2D / plot_3D to inspect spatial, temporal, or spectral evolution.
"""
...
x_resolution = 71  # Number of grid points in the x-direction
y_resolution = 71  # Number of grid points in the y-direction
t_resolution = 50  # Number of grid points in time (2D version of the code)
loop_inner_resolution = 25  # Inner loop resolution factor (M)
loop_outer_resolution = 1  # Outer loop resolution factor (Kmax)
# FOR TESTING you can set dz very small (1e-10) and then 3e-5 to see how it's changed
dz = 2e-5  # !!!!!!! total distance = dz * loop_inner_resolution * loop_outer_resolution  !!!!!!!
...
t_start, t_finish = 0, 1e-13  # t-range (0 to 1000e-13 s)

# Temporal domain boundaries (in seconds) for the split_step_time (over time)
# t_start, t_finish = -100e-13, 100e-13  # t-range (0 to 1000e-13 s)
...
# to see the difference you can swap Pmax 2e-3 with a low number like 1e-10
Pmax = 2e-3  # Beam power of the pulse (e.g., in Watts)
...
tau_0 = 1e-14  # Temporal parameter for the Hobbit field
...
if __name__ == '__main__':
    ...
    # 4) Propagate the field along t
    field_propagated = split_step_time(
        field_temp,
