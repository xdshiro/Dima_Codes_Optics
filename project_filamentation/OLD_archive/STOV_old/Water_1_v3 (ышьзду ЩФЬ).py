...
# %% Resolutions
xResolution = 281  # N_per
yResolution = 281
tResolution = 181
loopInnerResolution, loopOuterResolution = 401, 1  # M, Kmax
zResolution = loopInnerResolution * loopOuterResolution
xStart, xFinish = 0, 900 * 1e-6  # 10
yStart, yFinish = 0, 900 * 1e-6  # 10
zStart, zFinish = 0, 0.2
tStart, tFinish = 0, 700 * 1e-15
...
def Field_simple_OAM(x, y, t):
        return Imax * (np.exp(- (radius(x - x0, y - y0) ** 2) / rho0 ** 2 -
                              1j * k0 * radius(x - x0, y - x0) ** 2 / (2 * f) -
                              ((t - t0) / tp) ** 2)) * ((x - x0)/rho0 + 1j * np.sign(lOAM) * (y - y0)/rho0) ** np.abs(lOAM)
...
if __name__ == '__main__':
    ...
    if module_2D:
        ...
        plt.title(f't={round(time * 1e15, 0)}fs', fontweight="bold",
                  fontsize=26)
        plt.show()
        plt.close()
        np.save('test_OAM_abs.npy', fieldOLD)
        plt.show()
        exit()

    if module_HOBBIT:
        E = Field_simple_OAM(xytMesh[0], xytMesh[1], xytMesh[2])[:, :, int(tResolution / 2)]
        ...
        """fieldAdiZ = ADI_2D1_nonlinear_Z(Field_1_2D(xtMesh[0], xtMesh[1]),
                                     loopInnerResolution, loopOuterResolution)"""
        fieldOLD = split_step_old_time_Z(Field_simple_OAM, loopInnerResolution, loopOuterResolution)
        plot_2D(np.abs(fieldOLD[:, int(yResolution / 2), :]) ** Int, xArray * 1e3, zArray * 1e3, 'z (mm)', 'x (mm)',
                map='viridis')
        # plt.xlim(2.25 - 1, 2.25 + 1)
        # plt.ylim(2.25 - 1, 2.25 + 1)
        ...
        plt.show()
        plt.close()
        np.save('simple_small_energy_5MW.npy', fieldOLD)
        plt.show()
        exit()
    if 0:
