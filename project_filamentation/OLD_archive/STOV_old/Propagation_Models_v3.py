...
yResolution = 41
tResolution = 71
loopInnerResolution, loopOuterResolution = 71, 1  # M, Kmax
zResolution = loopInnerResolution * loopOuterResolution
xStart, xFinish = 0, 50 # 10
yStart, yFinish = 0, 50 # 10
zStart, zFinish = 0, 10000
tStart, tFinish = 0, 13
x0 = (xFinish - xStart) / 2
y0 = (yFinish - yStart) / 2
t0 = (tFinish - tStart) / 2
...
cSOL = 1
# w0 = 1
n2 = 1e0 * 2e-6  # 3 * chi3 / (4 * eps0 * c * n0 ** 2)
K = 1
eps0 = 1
n0 = 1.0
lOAM = 0
...
def split_step_old_time(shape, loopInnerM=1, loopOuterKmax=1):
    ...
    def Nonlinearity_spec(E):
        #return  dz * 1j * (w0 + KxywMesh[2]) * chi3 * E * I(E) / 2 /cSOL / eps0
        return dz * 1j * (w0 + KxywMesh[2]) * n2 * I(E) / cSOL - dz * Betta_func(K) * I(E) ** (K - 1)  # & E
        #return dz * (1j / 2 * eps0) * ((w0 + KxywMesh[2])/ cSOL / n0) * eps0 * chi3 * I(E)  - dz * Betta_func(K) * I(E) ** (K - 1)  # & E
...
if __name__ == '__main__':
    ...
    if 1:
        ...
        plot_1D(tArray * 3, np.abs(fieldOLD[int(xResolution/2), int(yResolution/2), :])
                , label='Paraxial', xname=r'$t$', yname='', color='lime', ls='-', lw=6)
        fieldUPPE = UPPE_time(Field_1, loopInnerResolution, loopOuterResolution)
        plot_1D(tArray * 3, np.abs(fieldUPPE[int(xResolution/2), int(yResolution/2), :])
