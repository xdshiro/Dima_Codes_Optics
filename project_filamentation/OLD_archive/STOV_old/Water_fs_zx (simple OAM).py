...
def main():
    ...
    def Field_STOV_simple(x, y, t):
        ...
        return Imax * (((t - t0) / tSTOVRadius + 1j * np.sign(lOAM) * (x - x0) / xSTOVRadius) ** (
    ...
    if module_2D:
        ...
        """fieldAdiZ = ADI_2D1_nonlinear_Z(Field_1_2D(xtMesh[0], xtMesh[1]),
                                     loopInnerResolution, loopOuterResolution)"""
        fieldOLD, zArrayWithTime = split_step_old_time_Z(Field_1, loopInnerResolution, loopOuterResolution)
        plot_2D(np.abs(fieldOLD[:, int(yResolution/2), :]) ** Int, xArray * 1e3, zArrayWithTime * 1e3, 'z (mm)', 'x (mm)',
                map='viridis')
        ...
        plt.show()
        plt.close()
        np.save('filam_simpleOAM_xz_75_25.npy', fieldOLD)
        plt.show()
        exit()
