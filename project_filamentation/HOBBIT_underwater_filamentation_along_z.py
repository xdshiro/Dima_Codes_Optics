"""
Same framework as HOBBIT_underwater_filamentation_along_CW.py, updated to visualize
beam evolution along the propagation axis (z) instead of time.

Key differences:
  • module_along_z flag enables x–z cross‑section plotting at a chosen time slice
  • time_to_look_at selects the temporal index for x–z maps
  • split_step_time_Z / UPPE_time generate (x, y, z) volumes
  • Example “__main__” workflow:
      1. Build a pulsed beam on (x, y, t) mesh
      2. Normalize to Pmax
      3. Propagate via split_step_time_Z over z steps
      4. Plot x vs. z slice at fixed y using plot_2D

Usage:
  • Toggle module_checking_spectrum and module_along_z for diagnostics
  • Set time_to_look_at and z_resolution as needed
  • Call split_step_time_Z(hobbit, M, K) and inspect propagation with plot_2D
"""
...
# it's supposed to be half of t_reso, but you can shift tou look at different times
time_to_look_at = 25  # which time slice to show at xz cross-section in between t_start and t_finish
loop_inner_resolution = 51  # Inner loop resolution factor (M)
...
t_start, t_finish = 0, 5e-13  # t-range (0 to 1000e-13 s)
...
def split_step_time_Z(shape: callable, loopInnerM: int = 1, loopOuterKmax: int = 1) -> np.ndarray:
    ...
    time_index = time_to_look_at  # Record the central time slice.
