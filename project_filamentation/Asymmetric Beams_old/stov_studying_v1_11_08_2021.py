...
def Field_STOV_simple(x, y, t):
    ...
    return ((1j * np.sign(lOAM) * (y - y0)/tSTOVRadius + gamma * (x - x0 - v * (t - t0)) / xSTOVRadius) ** (np.abs(lOAM))
            * np.exp(-1j * (w0 * gamma * ((t - t0) - v * x))))  # gamma * (t - t0 - v * x)
